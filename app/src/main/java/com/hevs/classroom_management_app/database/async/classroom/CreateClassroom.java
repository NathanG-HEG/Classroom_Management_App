...
public class CreateClassroom extends AsyncTask<Classroom, Void, Void> {
    private AppDatabase database;
    private OnAsyncEventListener callback;
    private Exception exception;

    public CreateClassroom(Context context, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(Classroom... classrooms) {
        try {
            for (Classroom c : classrooms) {
                database.classroomDao().insert(c);
            }
        } catch (Exception e) {
            exception = e;
        }
