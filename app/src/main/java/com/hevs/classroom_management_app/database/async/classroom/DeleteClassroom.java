...
public class DeleteClassroom extends AsyncTask<Classroom, Void, Void> {
    private AppDatabase database;
    private OnAsyncEventListener callback;
    private Exception exception;

    public DeleteClassroom(Context context, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(Classroom... classrooms) {
        try {
            for (Classroom c : classrooms) {
                database.classroomDao().delete(c);
            }
        } catch (Exception e) {
            exception = e;
        }
