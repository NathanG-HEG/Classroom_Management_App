...
public class UpdateReservation  extends AsyncTask<Reservation, Void, Void> {
    private AppDatabase database;
    private OnAsyncEventListener callback;
    private Exception exception;

    public UpdateReservation(Context context, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(Reservation... reservations) {
        try {
            for (Reservation r : reservations) {
                database.reservationDao().update(r);
            }
        } catch (Exception e) {
            exception = e;
        }
